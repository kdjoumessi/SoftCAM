...
#########
def build_resnet_model(cfg, pretrained=True):
    num_class = cfg.data.num_classes
    kaggle_weight = cfg.train.kaggle_weight
    dset = cfg.data.dataset

    print(f'kaggle weight: {kaggle_weight}')

    if pretrained:
        print('pretrained on ImageNet: weights=ResNet50_Weights.DEFAULT')
        res_model =models.resnet50(weights='ResNet50_Weights.DEFAULT') #models.resnet50(pretrained=True)
    else:
        res_model = models.resnet50()
    res_model.fc = nn.Linear(res_model.fc.in_features, num_class) 

    if kaggle_weight: 
        print(f'build the backbone resnet from the pretrained model on ImageNet + {dset}')
        if cfg.data.binary:
            cname = cfg.data.target_col_name 
            print(f'load binary {cname} bagnet weights')
            model_weights = torch.load(f'./pretrained_models/bin_{cname}_resnet_best_validation_weights_acc.pt')
        else:
            cname = 'multi_res' if cfg.data.dataset in ['kaggle'] else 'multi_res_oct'
            model_weights = torch.load(f'./pretrained_models/{cname}_best_validation_weights_acc.pt') #f'./pretrained_models/{cfg.train.checkpoint}'
            print('load multi-class resnet weights')
        res_model.load_state_dict(model_weights, strict=True)
        print(f'model weight: ', cname)
    else:
        print('build the backbone bagnet from the pretraine model on ImageNet')
 
    res_model = list(res_model.children())[:-2]     
    model = Resnet_model(res_model)
    return model
